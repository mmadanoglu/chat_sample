  ...
  const activityMiddleware = () => (next: any) => (card: any) => {
    ...
    if (card.activity) {
      ...
      if (
        ...
        console.log(
          `%c [ActivityMiddleware][Message][System] ${card.activity.text}`,
          "background: #2a9fd4; color: #fff"
        );
        return (children: any) => (
      ...
      if (card.activity.from.role === "bot") {
        console.log(
          `%c [ActivityMiddleware][Message][Agent] ${card.activity.text}`,
          "background: #2a9fd4; color: #fff"
        );
      }
      ...
      if (card.activity.from.role === "user") {
        console.log(
          `%c [ActivityMiddleware][Message][Customer] ${card.activity.text}`,
          "background: #2a9fd4; color: #fff"
        );
      }
    }
