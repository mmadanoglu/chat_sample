...
function WebChat() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (
        liveChatContext &&
        Object.keys(JSON.parse(liveChatContext)).length > 0
      ) {
  ...
  const startChat = useCallback(
    async (_, optionalParams = {}) => {
      ...
      store.subscribe(
        "DataMasking",
        createDataMaskingMiddleware(dataMaskingRules)
      );

      // Check for active conversation in cache
      const cachedLiveChatContext = localStorage.getItem("liveChatContext");
      if (
        cachedLiveChatContext &&
        Object.keys(JSON.parse(cachedLiveChatContext)).length > 0
      ) {
        console.log("[liveChatContext]");
      ...
      if (liveChatContext && Object.keys(liveChatContext).length) {
        localStorage.setItem(
          "liveChatContext",
          JSON.stringify(liveChatContext)
        );
      }
  ...
  return (
    <>
      <div>{!state.hasChatStarted && <ChatButton onClick={startChat} />}</div>
      {state.hasChatStarted && (
        <div className="chat-container">
          <ChatHeader title={"Live Chat via Chat SDK"} onClick={endChat} />
          {state.isLoading && <Loading />}
          {!callingConfig.disable && VoiceVideoCallingSDK && chatToken && (
            <Calling
          ...
          <div id="WebChat" style={{ height: "100%" }} />
          {!state.isLoading && state.hasChatStarted && chatAdapter && (
            <ActionBar
