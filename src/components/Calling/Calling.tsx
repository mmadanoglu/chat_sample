...
const adjustWebChatHeightInVideoCall = () => {
  ...
  const webChatTranscriptContainer = document.getElementsByClassName(
    "webchat__basic-transcript"
  )[0] as HTMLElement;
  const remoteVideoContainer = document.getElementById(
    "remoteVideo"
  ) as HTMLElement;
  const currentCallActionBar = document.getElementsByClassName(
    "current-call-action-bar"
  )[0] as HTMLElement;

  const newHeight =
    webChatTranscriptContainer.clientHeight -
    remoteVideoContainer.clientHeight -
    currentCallActionBar.clientHeight;
  webChatTranscriptContainer.style.marginTop = `${
    remoteVideoContainer.clientHeight + currentCallActionBar.clientHeight
  }px`;
...
const adjustWebChatHeightIncomingCall = () => {
  ...
  const incomingCallContainer = document.getElementsByClassName(
    "incoming-call-pop-up"
  )[0] as HTMLElement;

  const newHeight =
    webChatTranscriptContainer.clientHeight -
    incomingCallContainer.clientHeight;
  webChatTranscriptContainer.style.height = `${newHeight}px`;
};
...
const adjustWebChatHeightInVoiceCall = () => {
  ...
  const newHeight =
    webChatTranscriptContainer.clientHeight - currentCallActionBar.clientHeight;
...
function Calling(props: CallingProps) {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        console.error(
          "[WebChat][Calling] Failed to initialize VoiceVideoCalling!"
        );
        console.error(e);
      }
  ...
  const renderToggleVideoButton = () => {
    if (!isLocalVideoEnabled) {
      return (
        <VideoOff className="toggle-video-button" onClick={toggleVideoButton} />
      );
    }
    return (
  ...
  const renderToggleMuteButton = () => {
    if (isMicrophoneMuted) {
      return (
        <MicOff className="toggle-mute-button" onClick={toggleMuteButton} />
      );
    }
  ...
  return (
    <>
      ...
      {
        <div
          className={`calling ${inVideoCall || inVoiceCall ? "active" : ""}`}
        >
          ...
          <div>
            {(inVideoCall || inVoiceCall) && (
              <div className="current-call-action-bar">
                {renderToggleVideoButton()}
                {renderToggleMuteButton()}
                <PhoneOff
                  className="stop-call-button"
                  onClick={stopCallButton}
                />
              </div>
            )}
          </div>
